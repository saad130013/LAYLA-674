    ...
    <div className="space-y-6 bg-[#F5F7FA] dark:bg-gray-900 p-4 rounded-lg">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          <div className="lg:col-span-2 space-y-6">
              ...
              {/* 4. Supervisor Performance & Task Execution */}
              <div>
                <h2 className="text-xl font-bold text-[#1B4965] dark:text-white mb-4">{t('supervisorPerformance')}</h2>
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                    {/* Processing Time */}
                    <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                        <h4 className="font-semibold text-gray-600 dark:text-gray-300 flex items-center gap-2 text-sm"><Clock size={16}/> {t('avgProcessingTime')}</h4>
                    ...
                    {/* Task Execution (NEW) */}
                    <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                        <h4 className="font-semibold text-gray-600 dark:text-gray-300 flex items-center gap-2 text-sm"><CheckCheck size={16}/> {t('taskExecution')}</h4>
                        <div className="mt-2 flex justify-between items-end">
                            <div>
                                <p className="text-3xl font-bold text-brand-blue">{taskStats.completed}<span className="text-sm text-gray-400 font-normal">/{taskStats.total}</span></p>
                                <p className="text-xs text-green-600 font-semibold">{t('completionRate')}: {taskStats.rate}%</p>
                            </div>
                            <div className="text-right text-xs text-gray-500">
                                <p>{taskStats.pending} {t('scheduled')}</p>
                                <p>{taskStats.completed} {t('completed')}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Rejected Reports */}
                     <div className="p-4 bg-white dark:bg-gray-800 rounded-lg shadow-sm">
                        <h4 className="font-semibold text-gray-600 dark:text-gray-300 text-sm">{t('rejectedReports')}</h4>
